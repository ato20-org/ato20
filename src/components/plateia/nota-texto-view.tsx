...
import { FileText, Image as ImageIcon, NotebookPen, VenetianMask } from "lucide-react";
...
export type VinculosDaNota = {
  personagem: (nome: string) => { nome: string; dono: string; emCena: boolean } | null;
...
function TokenView({
  ...
  if (token.tipo === "personagem") {
    ...
    if (!achado) return <NaoResolvido bruto={token.bruto} alvo="personagem na mesa" />;
  ...
  if (token.tipo === "arquivo") {
    ...
    if (!arquivo) return <NaoResolvido bruto={token.bruto} alvo="arquivo seu" />;
    ...
    return (
      ...
      >
        ...
        {imagem ? <ImageIcon className={ICONE} aria-hidden /> : <FileText className={ICONE} aria-hidden />}
