...
export function InventarioJogador({
  ...
  return (
    <section className="space-y-1.5">
      ...
      <Dialog open={Boolean(aberto)} onOpenChange={(open) => !open && setAberto(null)}>
...
function ImagemDoItem({
  ...
  useEffect(() => {
    ...
    void characterFileThumbUrl(codigo, personagemId, imagem.autor, imagem.arquivo).then(
      (endereco) => {
  ...
  if (!url) return <Package className="text-muted-foreground/40 size-6 shrink-0" aria-hidden />;
...
function ItemForm({
  ...
  async function enviarFoto(file: File | undefined) {
    ...
    try {
      ...
      toast.error(cause instanceof Error ? cause.message : "Falha ao enviar a foto.");
    } finally {
      setEnviando(false);
  ...
  return (
    <>
      ...
      <DialogDescription className="sr-only">
        {meu ? "Nome, descrição, quantidade e foto do item." : "O que o mestre pôs no inventário."}
      </DialogDescription>
          ...
          <div className="flex items-center gap-2">
            ...
            {meu ? (
              <NumberField
                ...
                onValueCommitted={(valor) => void salvar({ quantidade: valor ?? 1 })}
              />
            ) : (
      ...
      {meu ? (
        ...
        <p className="text-muted-foreground text-sm whitespace-pre-wrap">{atual.descricao}</p>
      ) : null}
      ...
      {meu ? (
        <DialogFooter className="sm:justify-between">
          ...
          >
            <Camera /> {enviando ? "Enviando…" : atual.imagem ? "Trocar foto" : "Foto"}
          </Button>
        ...
        <p className="text-muted-foreground text-[10px]">Este item foi o mestre que pôs aqui.</p>
      )}
    </>
  );
}
