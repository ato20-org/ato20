...
import { PinTethers } from "@/components/mestre/pin-tethers";
 ...
 *
 * Mora aqui, no Mestre, e NÃO no `SceneLayer`. É deliberado: o `SceneLayer` é
 * o mesmo componente no Espectador, no Jogador e na miniatura, e um ponto de
  ...
  function iniciarGesto(
    ...
    startDrag(event, {
      ...
      onMove: (delta) => {
        if (
          !arrastou.current &&
          Math.hypot(delta.x, delta.y) * scale < LIMIAR_PX
        )
          return;
