...
export function RolagensFaixa() {
   ...
   */
  const inicio = useRef({ ponteiro: { x: 0, y: 0 }, lugar: { x: 0, y: 0, escala: 1 } });
  ...
  function lugarAtual(): LugarDaFaixa {
    ...
    if (!alvo || !(pai instanceof HTMLElement)) return lugar ?? { x: 0, y: 0, escala: 1 };
  ...
  return (
    <div
      ...
      style={
        lugar
          ? { left: lugar.x, top: lugar.y, ["--faixa-escala" as string]: escala }
      ...
      {bandeja.slice(0, TETO).map((rolagem) => {
        ...
        <div className="absolute -top-1 -left-6 opacity-0 transition-opacity group-hover/faixa:opacity-100 focus-within:opacity-100 motion-reduce:transition-none">
          <Tooltip>
            ...
            />
            <TooltipContent>
              <p className="max-w-48">Arraste para mover. Dois cliques devolvem ao topo.</p>
            </TooltipContent>
          </Tooltip>
        </div>
        ...
        <div className="absolute -right-6 -bottom-1 opacity-0 transition-opacity group-hover/faixa:opacity-100 focus-within:opacity-100 motion-reduce:transition-none">
          <Tooltip>
            ...
            />
            <TooltipContent>
              <p className="max-w-48">Arraste para baixo para aumentar, para cima para diminuir.</p>
            </TooltipContent>
          </Tooltip>
        </div>
      </div>
