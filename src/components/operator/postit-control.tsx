  ...
  return (
    <Popover>
      <Tooltip>
        <TooltipTrigger
          render={
            <PopoverTrigger
              render={
                <Button variant="ghost" size="icon-sm" aria-label="Cor do postit">
                  <span className={cn("size-4 rounded-[2px] ring-1", AMOSTRA[corPostit])} />
                </Button>
              }
            />
          }
      ...
      <PopoverContent align="start" className="w-52 space-y-2 p-3" side="top">
        <span className="text-muted-foreground text-[10px]">Cor do próximo postit</span>
          ...
          {CORES_POSTIT.map((cor) => (
            <button
              ...
              className={cn(
                ...
                AMOSTRA[cor],
                cor === corPostit ? "ring-foreground scale-110 ring-2" : "hover:scale-105",
              )}
        ...
        <p className="text-muted-foreground text-[10px] leading-snug">
          A cor separa assunto: pista, regra, fala de PNJ, lembrete. Trocar aqui não repinta os
          papéis já colados.
        </p>
      </PopoverContent>
    </Popover>
