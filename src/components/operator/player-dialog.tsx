  ...
  return (
    <Dialog open={Boolean(player)} onOpenChange={(open) => !open && onVoltar()}>
      <DialogContent className="sm:max-w-md">
        {player ? (
          <Ficha player={player} presente={presente} onVoltar={onVoltar} onChanged={onChanged} />
        ) : null}
      </DialogContent>
    </Dialog>
  );
...
function Ficha({
  ...
  useEffect(() => {
    ...
    void playerAttachments(player.id).then(
      async (lista) => {
        ...
        // Plateia também evita.
        const resolvidas = await Promise.all(
          lista
            .filter((anexo) => attachmentKind(anexo.arquivo, anexo.mimeType) === "image")
            .map(async (anexo) => {
              try {
                return [anexo.arquivo, await playerAttachmentUrl(player.id, anexo)] as const;
              } catch {
  ...
  useEffect(
    () => () => {
      for (const url of Object.values(abertas.current)) URL.revokeObjectURL(url);
      abertas.current = {};
    },
    [],
  );
  ...
  return (
    <>
      ...
      <section className="space-y-1.5">
        ...
        {anexos === null ? (
          <Loader2 className="text-muted-foreground size-4 animate-spin" aria-label="Carregando" />
        ) : anexos.length === 0 ? (
          <p className="text-muted-foreground text-xs">
            Nada anexado. Retrato, ficha, mapa rabiscado — ele anexa na aba Personagem.
          </p>
        ) : (
                  ...
                  return (
                    <li key={anexo.arquivo} className="relative">
                      ...
                      >
                        ...
                        <span className="block truncate p-1 text-[10px]" title={anexo.arquivo}>
                          {anexo.arquivo}
                        </span>
                      </button>
                        ...
                        <TooltipTrigger
                          render={
                            <Button
                              ...
                              onClick={() => (noAr ? limparEvidencia() : void transmitir(anexo))}
                            >
                  ...
                  return (
                    <li key={anexo.arquivo}>
                      <button
                        ...
                        onClick={() => setVendo(anexo)}
                      >
                        <Icone className="text-muted-foreground size-3.5 shrink-0" aria-hidden />
