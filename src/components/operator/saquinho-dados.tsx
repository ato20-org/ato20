 ...
 *
 * Só o mestre vê. Não há publicação: monta em `OperatorShell` e o store não
  ...
  function pegarBolinha(event: ReactPointerEvent) {
    ...
    screenDrag(event, {
      onMove: (delta) => {
        ...
        mover({
          x: Math.min(1 - folgaX, Math.max(folgaX, origem.x + delta.x / rect.width)),
          y: Math.min(1 - folgaY, Math.max(folgaY, origem.y + delta.y / rect.height)),
        });
      },
      onEnd: () => {
        setArrastando(false);
    ...
    <div className="space-y-3">
      ...
      {pousados.length >= 2 || (pousados.length >= 1 && noAr.size > 0) ? (
        <div className="flex items-baseline gap-2 border-t pt-2.5">
          <span className="text-muted-foreground flex-1 text-xs font-medium">Na mesa</span>
      ...
      {historico.length > 0 ? (
        <div className="space-y-1.5 border-t pt-2.5">
          <p className="text-muted-foreground text-xs font-medium">Últimas rolagens</p>
          ...
          <ul className="space-y-0.5">
            {historico.map((rolagem) => {
              ...
              return (
                <li key={rolagem.id} className="flex items-center gap-2 text-xs">
                  <span
                    aria-hidden
                    className="size-2.5 shrink-0 rounded-full border border-white/20"
                    style={{ background: tipo.hex }}
                  />
                  <span className="text-muted-foreground flex-1 truncate">{tipo.nome}</span>
                  <span
                    className={cn(
                  ...
                  >
                    {caindo ? "…" : valorDaRolagem(rolagem.faces, rolagem.valor)}
                  </span>
...
function useDadosNoAr(dados: Dado[]): ReadonlySet<string> {
  ...
  useEffect(() => {
    ...
    const recalcular = () => {
      ...
      setNoAr(new Set(dados.filter((_, i) => restante[i] > 0).map((dado) => dado.id)));
      ...
      if (proximo.length > 0) timer = setTimeout(recalcular, Math.min(...proximo) + 16);
    };
