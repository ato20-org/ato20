    ...
    <Popover>
      <Tooltip>
        <TooltipTrigger
          render={
            <PopoverTrigger
              render={
                <Button variant="ghost" size="icon-sm" aria-label="Cor e espessura">
                  <span
      ...
      <PopoverContent align="start" className="w-56 space-y-3 p-3" side="top">
        ...
        <div className="space-y-1.5">
          ...
          <Slider
            value={[ESPESSURAS_LAPIS.indexOf(espessura as (typeof ESPESSURAS_LAPIS)[number])]}
            min={0}
        ...
        {apagandoTudo.length > 0 && scene ? (
          <Button
            ...
            size="sm"
            className="w-full"
            onClick={() => removeTracos(scene.id, apagandoTudo.map((traco) => traco.id))}
          >
            <Trash2 />
