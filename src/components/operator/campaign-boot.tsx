...
import { CampaignSplash, type BootStep } from "@/components/operator/campaign-splash";
 ...
 *
 * Existe separado do `OperatorShell` porque a ordem importa e o shell não podia
...
export function CampaignBoot({ campaign }: { campaign: CampaignInfo }) {
  ...
  useEffect(() => {
    ...
    async function abrir() {
      ...
      try {
        ...
        await Promise.all([hydratePortraits(campaign.path), hydrateTrack(campaign.path)]);
        if (!ativo) return;
        setFase("acervo");
        ...
        await Promise.all([listAssets("image"), listAssets("audio"), listFolders()]);
        if (!ativo) return;
        ...
        if (ativo) setErro(cause instanceof Error ? cause.message : "Falha ao abrir a campanha");
      }
    }
  ...
  if (fase === "pronto") return <OperatorShell />;
