...
export function InventarioPersonagem({
  ...
  const recarregar = useCallback(() => {
    listInventory(personagem.id).then(setItens, (cause: unknown) => {
      toast.error(cause instanceof Error ? cause.message : "Falha ao ler o inventário.");
      setItens([]);
    });
  }, [personagem.id]);
  ...
  async function receber(de: string, itemId: string) {
    try {
      ...
      toast.error(cause instanceof Error ? cause.message : "Falha ao mover o item.");
    }
  }
  ...
  async function criar() {
    try {
      ...
      toast.error(cause instanceof Error ? cause.message : "Falha ao criar o item.");
    }
  }
  ...
  // grade antes.
  const adicionar = (
    <Button variant="ghost" size="icon-xs" aria-label="Adicionar item" onClick={() => void criar()}>
      <Plus />
    </Button>
  );
    ...
    >
      <Grade
        ...
        onDrop={(event) => {
          ...
          if (!payload?.item || payload.item.personagemId === personagem.id) return;
...
const ITEM_MENU = "text-xs whitespace-nowrap [&_svg:not([class*='size-'])]:size-3.5";
...
function ItemTile({
  ...
  const { noAr, alternar: alternarTransmissao } = useTransmissaoDoItem(personagemId, item);
  ...
  async function mexer(quanto: number) {
    ...
    try {
      ...
      toast.error(cause instanceof Error ? cause.message : "Falha ao mudar a quantidade.");
    } finally {
      setOcupado(false);
    }
  }
  ...
  async function esconder(escondido: boolean) {
    ...
    try {
      ...
      toast.error(cause instanceof Error ? cause.message : "Falha ao mudar o item.");
    } finally {
      setOcupado(false);
    }
  }
  ...
  async function apagar() {
    try {
      ...
      toast.error(cause instanceof Error ? cause.message : "Falha ao remover o item.");
    }
  }
    ...
    >
      ...
      <div
        className={cn(
          ...
          menuAberto ? "block" : "hidden group-hover/item:block group-focus-within/item:block",
        )}
      >
        <DropdownMenu open={menuAberto} onOpenChange={setMenuAberto}>
          ...
          <DropdownMenuContent align="end">
            {item.imagem ? (
              <DropdownMenuItem className={ITEM_MENU} onClick={alternarTransmissao}>
...
function ImagemDoItem({
  ...
  if (!imagem || falhou || !url) {
    return (
      <Package className="text-muted-foreground/40 size-6 shrink-0" aria-hidden />
    );
  }
...
function ItemForm({
  ...
  async function salvar(patch: Parameters<typeof updateItem>[2]) {
    try {
      ...
      toast.error(cause instanceof Error ? cause.message : "Falha ao salvar o item.");
    }
  }
    ...
    <>
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <Input
              ...
              onBlur={(event) => void salvar({ quantidade: Number(event.target.value) || 1 })}
            />
          </div>
        </div>
      </div>
        ...
        {outros.length > 0 ? (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="start">
              {outros.map((destino) => (
                <DropdownMenuItem key={destino.id} onClick={() => void mover(destino)}>
                  {destino.nome}
                </DropdownMenuItem>
              ))}
            </DropdownMenuContent>
      ...
      <DialogFooter className="sm:justify-between">
        <label className="flex items-center gap-2 text-xs">
          ...
          />
          {atual.escondido ? <EyeOff className="size-3.5" /> : <Eye className="size-3.5" />}
          {atual.escondido ? "Escondido do jogador" : "O jogador vê"}
        </label>

        <Button variant="ghost" size="sm" className="text-destructive" onClick={() => void apagar()}>
          <Trash2 /> Remover
        </Button>
      </DialogFooter>
